      ...
      assets = xml.readNodes("/eveapi/result/rowset/row").map do |asset|
        {
          ...
          :contained_items => asset.readNodes("rowset/row").map do |item|
            {
      ...
      orders = xml.readNodes("/eveapi/result/rowset/row").map do |order|
        {
          ...
          :original_volume => order.readAttribute("volEntered").integerValue,
          :remaining_volume => order.readAttribute("volRemaining").integerValue,
          :minimum_volume => order.readAttribute("minVolume").integerValue,
      ...
      jobs = xml.readNodes("/eveapi/result/rowset/row").map do |job|
        {
          ...
          :container => {
            :id => job.readAttribute("containerID").integerValue,
            :type_id => job.readAttribute("containerLocationID").integerValue,
            :location_id => job.readAttribute("containerTypeID").integerValue
          },
          :installed_item => {
            :id => job.readAttribute("installedItemID").integerValue,
            :type_id => job.readAttribute("installedItemTypeID").integerValue,
            :location_id => job.readAttribute("installedItemLocationID").integerValue,
            :quantity => job.readAttribute("installedItemQuantity").integerValue,
            :productivity_level => job.readAttribute("installedItemProductivityLevel").integerValue,
            :material_level => job.readAttribute("installedItemMaterialLevel").integerValue,
            :runs_remaining => job.readAttribute("installedItemLicensedProductionRunsRemaining").integerValue,
            :copy => (job.readAttribute("installedItemCopy") == 1),
            :flags => parse_inventory_flags(job.readAttribute("installedItemFlag").integerValue)
          },
          :output => {
            :type_id => job.readAttribute("outputTypeID").integerValue,
            :location_id => job.readAttribute("outputLocationID").integerValue,
            :flags => parse_inventory_flags(job.readAttribute("outputFlag").integerValue)
          },
          :runs => job.readAttribute("runs").integerValue,
          :material_multiplier => job.readAttribute("materialMultiplier").floatValue,
          :character_material_multiplier => job.readAttribute("charMaterialMultiplier").floatValue,
          :time_multiplier => job.readAttribute("timeMultiplier").floatValue,
          :character_time_multiplier => job.readAttribute("charTimeMultiplier").floatValue,
      ...
      transactions = xml.readNodes("/eveapi/result/rowset/row").map do |transaction|
        {
          ...
          :type_id => transaction.readAttribute("typeID").integerValue,
          :type_name => transaction.readAttribute("typeName").stringValue,
          :price => transaction.readAttribute("price").floatValue,
          :client_id => transaction.readAttribute("clientID").integerValue,
          :client_name => transaction.readAttribute("clientName").stringValue,
          :station_id => transaction.readAttribute("stationID").integerValue,
          :station_name => transaction.readAttribute("stationName").stringValue,
          :transaction_type => transaction.readAttribute("transactionType").stringValue.intern,
          :transaction_for => transaction.readAttribute("transactionFor").stringValue.intern
        }
      end
      ...
      entries = xml.readNodes("/eveapi/result/rowset/row").map do |entry|
        {
          ...
          :first_id => entry.readAttribute("ownerID1").integerValue,
          :first_name => entry.readAttribute("ownerName1").stringValue,
          :second_id => entry.readAttribute("ownerID2").integerValue,
          :second_name => entry.readAttribute("ownerName2").stringValue,
          :argument_id => entry.readAttribute("argID1").integerValue,
          :argument_name => entry.readAttribute("argName1").stringValue,
